...
var DEFAULT_TAG = 'div';
var BLOCK_I_BEM = 'i-bem';
var ONCLICK_ATTR = 'onclick';

var MOD_VAL_BOOLEAN_ORIG = 'yes';
var MOD_VAL_BOOLEAN_NEW = true;


var modValBoolean = MOD_VAL_BOOLEAN_NEW;
...
function _bemNode(matched) {
    ...
    if (matched[3]) {
        var modName = matched[3],
            modVal = (typeof matched[4] !== 'undefined' ? matched[4] : modValBoolean);
...
function parseTree($, root, stack) {
    ...
    $(root).each(function() { // including text nodes
        ...
        if (!cheerioUtils.isTag($el[0])) {
            if ($el[0].type === 'text') { // inore 'comment' and etc.
                var text = _string.trim($el[0].data);
                if (text.length) {
                    content.push(text);
                }
            }
        ...
        if ($el[0].name !== DEFAULT_TAG) {
            node.tag = $el[0].name;
        }

        var attribs = {};
        for (var name in $el[0].attribs) {
            if (Object.prototype.hasOwnProperty.call($el[0].attribs, name)) {
                switch (name) {
                    case ONCLICK_ATTR:
                        break;
                    case CLASS_ATTR:
                        var clsList = $el.attr(name).split(/\s+/),
                        ...
                        if (bemNames.length) {
                            var jsParams = false;
                            try {
                                jsParams = _bemParseParams($el.attr(ONCLICK_ATTR));
                            } catch (e) {
                                attribs[ONCLICK_ATTR] = $el.attr(ONCLICK_ATTR);
                            }
        ...
        var nodeContent = parseTree($, $el[0].children, stack);
        if (nodeContent) {
